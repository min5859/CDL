...
class TabImageViewer:
    def __init__(self):
        # Add a dictionary to map format strings to decoding functions
        self.decoding_functions = {
            "RAW10": self._decode_raw10,
            "NV12": self._decode_nv12,
            "NV21": self._decode_nv21,
            "I420": self._decode_i420,
            "YUYV": self._decode_yuyv,
        }

    def _decode_raw10(self, data, width, height, stride):
        ...
        # 4 pixels are packed into 5 bytes (40 bits)
    ...
    def _decode_yuv(self, data, width, height, stride, conversion_code):
    ...
    def _decode_nv12(self, data, width, height, stride):
        return self._decode_yuv(data, width, height, stride, cv2.COLOR_YUV2RGB_NV12)

    def _decode_nv21(self, data, width, height, stride):
        return self._decode_yuv(data, width, height, stride, cv2.COLOR_YUV2RGB_NV21)

    def _decode_i420(self, data, width, height, stride):
        return self._decode_yuv(data, width, height, stride, cv2.COLOR_YUV2RGB_I420)

    def _decode_yuyv(self, data, width, height, stride):
        # For YUYV, OpenCV uses the YUY2 code
        return self._decode_yuv(data, width, height, stride, cv2.COLOR_YUV2RGB_YUY2)
    ...
    def render(self):
        ...
        image_format = st.sidebar.selectbox(
            "Select Image Format",
            list(self.decoding_functions.keys())
        )
        ...
        if display_button and uploaded_file is not None:
            ...
            decoder_func = self.decoding_functions.get(image_format)
